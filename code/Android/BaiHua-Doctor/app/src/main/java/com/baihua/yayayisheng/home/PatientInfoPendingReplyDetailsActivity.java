 ...
 * Description: 患者信息详情
 */
public class PatientInfoPendingReplyDetailsActivity extends BaseActivity {
    ...
    public void setLayout() {
        setTitle("患者信息");
        setContentView(R.layout.activity_patient_info_pending_reply_details);
    }
    ...
    @OnClick({R.id.play_voice_layout, R.id.layout_reply_need_to_check, R.id.patient_details_tv_submit})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.play_voice_layout:
                toast("TODO");
                break;
            case R.id.layout_reply_need_to_check:
                showCheckedDialog();
                break;
            case R.id.patient_details_tv_submit:
                toast("TODO");
                break;
        }
    }
        ...
        checkboxDialog = new MaterialDialog.Builder(this)
                ...
                .itemsCallbackMultiChoice(null, new MaterialDialog.ListCallbackMultiChoice() {
                    ...
                    public boolean onSelection(MaterialDialog dialog, Integer[] which, CharSequence[] text) {
                        ...
                        for (Integer integer :
                                which) {
                            String ends = text[integer].toString().split(" ")[1];
                            String price = ends.replaceAll("元", "");
                            total += Double.valueOf(price);
                        }
                        checkboxDialog.setContent(String.format("共计%s元", total));
                        LogUtils.e(Arrays.toString(which));
                        LogUtils.e(text[0] + Arrays.toString(text));
                        return true;
                    }
                })
