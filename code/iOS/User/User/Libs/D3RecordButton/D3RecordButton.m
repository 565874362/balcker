...
-(void)recording:(float)recordTime volume:(float)volume{
    ...
    if ([_delegate respondsToSelector:@selector(recording:)]) {
        [_delegate recording:recordTime];
    }

    if (_state == 1) {
        [RecordHUD setImage:@"mic_cancel.png"];
    } else {
