...
{
    if (_clickImg) {
        _clickImg(tap.view.tag,(UIImageView *)tap.view);
    }
}
...
- (void)setImageArray:(NSArray *)imageArray
{
    ...
    for(int index = 0; index< imageArray.count; index++) {
        ...
        UIImageView *cellView = [[UIImageView alloc ]init ];
        [cellView sd_setImageWithURL:[NSURL URLWithString:imageArray[index]] placeholderImage:[UIImage imageNamed:@"placeholderImage"]];
        cellView.clipsToBounds = YES;
        cellView.tag = index;
        //        cellView.layer.cornerRadius = 5.0f;
        cellView.userInteractionEnabled = YES;
